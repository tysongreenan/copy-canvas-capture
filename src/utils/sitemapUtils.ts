...
export function simplifyEdges(edges: any[]): any[] {
...
// Function to find the home node in the sitemap data
export function findHomeNode(nodes: any[], crawlUrl?: string) {
  return nodes.findIndex(node => 
    ...
    node.id === 'main' || 
    (crawlUrl && node.data.url === crawlUrl)
  );
}

// Function to create simplified edges between levels
export function createSimplifiedEdges(
  currentLevelNodes: string[],
  nextLevelNodes: string[], 
  originalEdges: any[]
): any[] {
  if (nextLevelNodes.length === 0) return [];
  
  const simplifiedEdges: any[] = [];
  
  // Find actual connections between these levels in the original edges
  const levelConnections = originalEdges.filter(edge => 
...
export function organizeSitemapData(sitemapData: SitemapData): SitemapData {
  ...
  const homeNodeIndex = findHomeNode(nodes);
  ...
  if (homeNodeIndex !== -1) {
    ...
    for (let level = 1; level < Math.max(...levelGroups.keys()); level++) {
      ...
      const levelEdges = createSimplifiedEdges(currentLevelNodes, nextLevelNodes, edges);
      simplifiedEdges.push(...levelEdges);
    }
