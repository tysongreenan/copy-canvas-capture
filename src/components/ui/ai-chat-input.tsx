  ...
  return (
    <div className="w-full flex justify-center items-center text-black">
      <motion.div
        ...
        initial="collapsed"
        style={{ overflow: "hidden", borderRadius: 32, background: "rgba(255, 255, 255, 0.08)" }}
        onClick={handleActivate}
      >
        <div className="flex flex-col items-stretch w-full h-full">
          {/* Input Row */}
          <div className="flex items-center gap-2 p-3 rounded-full max-w-3xl w-full bg-white/5">
            <button
                ...
                <AnimatePresence mode="wait">
                  {showPlaceholder && !isActive && !inputValue && (
                    <motion.span
                      ...
                      exit="exit"
                    >
                      {(placeholder || DEFAULT_CHAT_PLACEHOLDERS[placeholderIndex])
                        .split("")
                        .map((char, i) => (
                          <motion.span
                            key={i}
            ...
            <div className="flex gap-3 items-center">
              ...
              <button
                className={`flex items-center gap-1 px-4 py-2 rounded-full transition-all font-medium group ${
                  thinkActive
                    ? "bg-blue-600/20 outline outline-blue-400/60 text-blue-200"
              ...
              >
                <Lightbulb
                  className={`${thinkActive ? "text-yellow-300" : ""} group-hover:text-yellow-300 transition-all`}
                  size={18}
                />
                Think
              </button>
